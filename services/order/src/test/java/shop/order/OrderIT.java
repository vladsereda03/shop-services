...
class OrderIT {
  ...
  @Autowired private MockRestServiceServer cartServer;
  ...
  void checkoutTurnsCartIntoOrderAndClearsCartLast() throws Exception {
    ...
    mockMvc
        .perform(post("/orders/my").with(jwt().jwt(jwt -> jwt.claim("uid", USER_ID))))
  ...
  void myOrdersReturnsOnlyOwnOrdersNewestFirst() throws Exception {
    Order older = orderRepository.saveAndFlush(orderOf(USER_ID, Instant.now().minusSeconds(3600)));
    Order newer = orderRepository.saveAndFlush(orderOf(USER_ID, Instant.now()));
    orderRepository.saveAndFlush(orderOf(77L, Instant.now()));

    mockMvc
        .perform(get("/orders/my").with(jwt().jwt(jwt -> jwt.claim("uid", USER_ID))))
        .andExpect(status().isOk())
  ...
  static class CartClientStubConfig {
    ...
    @Bean
    @Primary
    RestClient cartStubRestClient(
        RestClient.Builder cartStubRestClientBuilder, MockRestServiceServer cartServer) {
